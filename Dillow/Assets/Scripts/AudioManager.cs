...
public class AudioManager : MonoBehaviour {
	...
	void Awake () {
		...
		InitAudioSource(sources[0]);
		sources[1] = gameObject.AddComponent<AudioSource>();
		InitAudioSource(sources[1]);
	}

	private MusicTrack FindTrack (string trackName) {
	...
	public static void PlayTrack (string track, bool loop = true, bool crossfade = true, float fadeTime = 3f, bool syncTimes = false) {
		PlayTrack(instance.FindTrack(track), loop, crossfade, fadeTime, syncTimes);
	}
