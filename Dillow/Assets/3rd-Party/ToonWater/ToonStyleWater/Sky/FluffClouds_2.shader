...
Category {
	...
	SubShader {
		Pass {
			...
			col.rgb*=_LightColor0.rgb*(5.5-saturate(col.a)*2);    // Make thin clouds brighter
...
float power = 256-ca*255;
float sun = pow(max(0,fdist), power)*(1-ca);
...
col.rgb += ((colDet.rgb*col.a)*fdist + radial
	)*_Directional.rgb ;
